 ...
 * webhook deliveries, notifications, punishments, sessions, etc. Lives next to
 * users.json and config.json in the manager-data volume so a single
 * `docker volume rm` wipes everything.
 *
 * Single connection per process, opened lazily, WAL mode for concurrent reads
 * while a writer holds the journal.
 *
 * Schema changes go through the MIGRATIONS list below — never edit an existing
 * migration's `up` once it has shipped. Add a NEW migration with the next
 * version number (ALTER TABLE / CREATE TABLE …). Applied versions are recorded
 * in schema_migrations, so each migration runs exactly once per database and
 * upgrades apply deterministically across every install. The baseline (v1) is
 * idempotent (CREATE TABLE IF NOT EXISTS), so pre-migration databases simply
 * record v1 on first boot without disruption.
 */
...
const MIGRATIONS: Migration[] = [
  ...
  // Future schema changes: add { version: 2, name: '…', up: (conn) => conn.exec('ALTER TABLE …') }
];

/**
 * Apply every migration whose version is greater than the highest already
 * recorded. Each runs inside a transaction together with its bookkeeping row,
 * so a failure rolls the whole migration back and leaves the version untouched.
 */
export function runMigrations(conn: Database.Database): void {
  ...
  `);
  const row = conn.prepare('SELECT MAX(version) AS v FROM schema_migrations').get() as { v: number | null };
  const current = row?.v ?? 0;
  const pending = MIGRATIONS.filter((m) => m.version > current).sort((a, b) => a.version - b.version);
  for (const m of pending) {
    const apply = conn.transaction(() => {
      m.up(conn);
      conn.prepare('INSERT INTO schema_migrations (version, name, applied_at) VALUES (?, ?, ?)')
...
/** Current schema version (highest applied migration), or 0 on a fresh DB. */
export function getSchemaVersion(conn: Database.Database = getDb()): number {
  const has = conn.prepare(

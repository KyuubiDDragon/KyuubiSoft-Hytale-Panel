    ...
    >
      <div v-if="open" class="absolute right-0 top-full mt-2 w-80 rounded-xl bg-dark-200 border border-dark-50/60 shadow-2xl z-50 overflow-hidden">
        <div class="px-3 py-2 border-b border-dark-50/60 flex items-center justify-between">
          ...
          <button v-if="notifications.length" @click="dismissAll" class="text-xs text-gray-400 hover:text-white">Dismiss all</button>
        </div>
        <div class="max-h-96 overflow-y-auto">
          <div v-if="notifications.length === 0" class="p-6 text-center text-sm text-gray-400">No notifications</div>
          <button

...
import { serverApi, type PluginStatus } from '@/api/server'
...
const showBanner = computed(() => {
  ...
  // Show banner if plugin is not installed or not running (when server is running)
  return !pluginStatus.value.installed || !pluginStatus.value.running
})
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1">
        <h3 class="font-semibold text-base">
          {{ !pluginStatus?.installed ? t('dashboard.plugin.notInstalled') : t('dashboard.plugin.notRunning') }}
        </h3>

...
<template>
  <div class="space-y-6" @click="closeDropdown">
      ...
      <div>
        ...
        <div class="flex items-center gap-2 mt-1">
          <p class="text-ink-muted">
            ...
            <span class="text-ink-subtle ml-2">({{ allPlayers.length }} {{ t('players.total') }})</span>
          </p>
          <span
    ...
    <Card v-if="activeTab === 'online'" :padding="false">
      ...
      <div v-else-if="onlinePlayers.length === 0" class="text-center text-ink-subtle p-8">
        ...
        >
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm text-ink-subtle">
                ...
                <span v-if="player.world" class="flex items-center gap-1">
                  <svg class="w-3.5 h-3.5 text-ink-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              ...
              class="p-2 text-ink-muted hover:text-green-400 transition-colors"
            ...
            <button
              ...
              class="p-2 text-ink-muted hover:text-yellow-400 transition-colors"
            ...
            <button
              ...
              class="p-2 text-ink-muted hover:text-red-400 transition-colors"
              :title="t('players.ban')"
            >
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="relative">
              ...
              >
                ...
                <button v-if="authStore.hasPermission('players.op')" @click="handleDeop(player.name)" class="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.618 5.984A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                  {{ t('players.deop') }}
                </button>
                <div v-if="authStore.hasPermission('players.op')" class="border-t border-border my-1"></div>
                <button v-if="authStore.hasPermission('players.gamemode')" @click="openGamemodeModal(player.name)" class="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button v-if="authStore.hasPermission('players.give')" @click="openGiveModal(player.name)" class="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" />
                  </svg>
                  {{ t('players.give') }}
                </button>
                <div v-if="authStore.hasAnyPermission('players.gamemode', 'players.give')" class="border-t border-border my-1"></div>
                <button v-if="authStore.hasPermission('players.respawn')" @click="handleRespawn(player.name)" class="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  {{ t('players.respawn') }}
                </button>
                <button v-if="authStore.hasPermission('players.effects')" @click="handleClearEffects(player.name)" class="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
                  </svg>
                  {{ t('players.clearEffects') }}
                </button>
                <div v-if="authStore.hasAnyPermission('players.respawn', 'players.effects')" class="border-t border-border my-1"></div>
                <button v-if="authStore.hasPermission('players.whitelist')" @click="handleWhitelist(player.name)" class="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  {{ t('players.addToWhitelist') }}
                </button>
                <div v-if="authStore.hasPermission('players.whitelist')" class="border-t border-border my-1"></div>
                <button v-if="authStore.hasPermission('players.kill')" @click="openKillModal(player.name)" class="w-full px-4 py-2 text-left text-sm text-red-400 hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                  {{ t('players.kill') }}
                </button>
                <button v-if="authStore.hasPermission('players.clear_inventory')" @click="openClearInventoryModal(player.name)" class="w-full px-4 py-2 text-left text-sm text-red-400 hover:bg-surface-overlay flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <Card v-if="activeTab === 'offline'" :padding="false">
      ...
      <div v-else-if="offlinePlayers.length === 0" class="text-center text-ink-subtle p-8">
        <svg class="w-12 h-12 mx-auto mb-3 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
        ...
        >
          ...
          >
            <div class="w-10 h-10 bg-gray-500/20 rounded-lg flex items-center justify-center group-hover:bg-gray-500/30 transition-colors">
              <span class="text-ink-muted font-bold">{{ player.name[0].toUpperCase() }}</span>
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <p class="font-medium text-white group-hover:text-hytale-orange transition-colors">{{ player.name }}</p>
              ...
              <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm text-ink-subtle mt-1">
    ...
    <Modal :open="showBanModal" :title="t('players.ban')" @close="showBanModal = false">
      <div class="space-y-4">
        ...
        <div>
          <label class="block text-sm text-ink-muted mb-2">{{ t('players.banReason') }}</label>
          <input
            v-model="banReason"
            type="text"
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-2">
          <button
            ...
            :class="['flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-colors', teleportMode === 'player' ? 'bg-hytale-orange text-dark' : 'bg-surface-overlay text-ink-muted']"
          >
            {{ t('players.teleportTo') }}
          </button>
          <button
            @click="teleportMode = 'coords'"
            :class="['flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-colors', teleportMode === 'coords' ? 'bg-hytale-orange text-dark' : 'bg-surface-overlay text-ink-muted']"
          >
            {{ t('players.teleportCoords') }}
          </button>
          <button
            @click="selectDeathTeleportMode"
            :class="['flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-colors', teleportMode === 'death' ? 'bg-red-500 text-white' : 'bg-surface-overlay text-ink-muted']"
          >
        ...
        <div v-else-if="teleportMode === 'coords'" class="grid grid-cols-3 gap-3">
          <div>
            <label class="block text-sm font-medium text-ink-muted mb-2">X</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-ink-muted mb-2">Y</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-ink-muted mb-2">Z</label>
            <input
        ...
        <div v-else-if="teleportMode === 'death'" class="space-y-3">
          <div v-if="deathPositionLoading" class="flex items-center justify-center py-4">
            <svg class="w-5 h-5 animate-spin text-ink-muted" fill="none" viewBox="0 0 24 24">
          ...
          <div v-else-if="lastDeathPosition" class="p-4 bg-surface-overlay rounded-lg space-y-2">
    ...
    <Modal :open="showGiveModal" :title="t('players.give') + ': ' + selectedPlayer" @close="showGiveModal = false">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-ink-muted mb-2">{{ t('players.itemId') }}</label>
          <ItemAutocomplete
            v-model="giveItem"
            :placeholder="t('players.searchItems')"
          />
          <p class="mt-1 text-xs text-ink-subtle">{{ t('players.itemIdHint') }}</p>
        </div>
        <div>
          <label class="block text-sm font-medium text-ink-muted mb-2">{{ t('players.amount') }}</label>
          <input

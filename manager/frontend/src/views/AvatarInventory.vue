...
<template>
  <div class="h-full flex flex-col bg-inv-bg">
    <!-- Header with search -->
    <div class="bg-inv-panel border-b border-inv-border/30 px-4 py-2">
      <div class="flex items-center justify-between">
        ...
        <div class="relative w-64">
          <div class="relative">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div
            ...
            class="absolute top-full left-0 right-0 mt-1 bg-inv-panel border border-inv-border/50 rounded-lg shadow-xl max-h-64 overflow-y-auto z-50"
          >
            <button
              v-for="player in filteredPlayers"
              :key="player.uuid"
              class="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-inv-panel-light transition-colors text-left"
    ...
    <div class="flex-1 overflow-auto p-3">
      ...
      <div v-if="!selectedPlayer" class="flex flex-col items-center justify-center h-full text-center">
        ...
        <h2 class="text-xl font-semibold text-ink-muted mb-2">{{ t('avatarInventory.selectPrompt') }}</h2>
        <p class="text-ink-subtle">{{ t('avatarInventory.selectHint') }}</p>
      </div>
      ...
      <div v-else-if="error" class="flex flex-col items-center justify-center h-full text-center">
        ...
        <p class="text-ink-muted text-lg">{{ error }}</p>
      </div>
      ...
      <div v-else class="flex gap-3 max-w-7xl mx-auto h-full">
        ...
        <div class="w-[360px] flex-shrink-0 flex flex-col gap-2">
          ...
          <div class="inv-panel p-3 relative">
            <div class="flex gap-3">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div
                  ...
                  :key="`armor-${index}`"
                  class="inv-slot border-2"
                  :class="[
                    item ? 'inv-slot-filled ' + getItemRarityBorder(item.itemId) : 'border-inv-border-light/30'
                  ]"
                ...
                >
                  <template v-if="item">
                    ...
                    <!-- Durability bar -->
                    <div v-if="item.maxDurability > 0" class="absolute bottom-0 left-0 right-0 h-0.5 bg-inv-bg rounded-b overflow-hidden">
              ...
              <div class="flex-1 relative">
                <div class="aspect-[3/4] max-h-[280px] bg-gradient-to-b from-dark-100/50 to-dark-500/80 rounded-lg border border-inv-border/30 overflow-hidden relative">
                  ...
                  <div v-if="avatarError" class="absolute inset-0 flex flex-col items-center justify-center text-inv-border">
                ...
                <div class="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 text-ink-subtle">
            ...
            <div class="flex justify-center gap-2 mt-3">
              ...
              <div class="relative group">
                <div
                  class="w-10 h-10 border-2 border-amber-500/30 bg-inv-slot rounded-lg flex items-center justify-center cursor-pointer hover:border-amber-500/60 transition-all"
                ...
                <div
                  ...
                  class="absolute left-full top-0 ml-1.5 flex gap-1 p-1.5 bg-inv-panel/95 border border-inv-border/50 rounded-lg shadow-xl z-10"
                  @mouseleave="expandedQuickSlot = null"
                >
                  <div v-for="i in 4" :key="i" class="w-8 h-8 border border-amber-500/30 bg-inv-slot-fill rounded flex items-center justify-center">
              ...
              <div class="relative group">
                <div
                  class="w-10 h-10 border-2 border-purple-500/30 bg-inv-slot rounded-lg flex items-center justify-center cursor-pointer hover:border-purple-500/60 transition-all"
                ...
                >
                  <div
                    ...
                    :class="[
                      item ? getItemRarityBorder(item.itemId) + ' bg-inv-slot-fill hover:bg-inv-panel-light' : 'border-purple-500/30 bg-inv-slot-fill'
                    ]"
                    @mouseenter="item && showTooltip(item, $event)"
                    @mouseleave="hideTooltip"
                  >
              ...
              <div class="relative group">
                <div
                  class="w-10 h-10 border-2 border-green-500/30 bg-inv-slot rounded-lg flex items-center justify-center cursor-pointer hover:border-green-500/60 transition-all"
                ...
                <div
                  ...
                  @mouseleave="expandedQuickSlot = null"
                >
                  <div v-for="i in 4" :key="i" class="w-8 h-8 border border-green-500/30 bg-inv-slot-fill rounded flex items-center justify-center">
          ...
          <div class="inv-panel p-3">
            <h3 class="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">{{ t('avatarInventory.stats') }}</h3>
            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-2">
              ...
              <div class="flex items-center gap-2">
                ...
                <div class="flex-1">
                  <div class="h-2.5 bg-inv-bg rounded-full overflow-hidden border border-inv-border/30">
              ...
              <!-- Defense -->
              <div class="flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex-1 flex flex-col gap-2 min-w-0">
          <!-- Tab Navigation -->
          <div class="flex gap-1 bg-inv-panel p-1 rounded-lg border border-inv-border/30">
            <button
          ...
          <div v-if="activeTab === 'inventory'" class="flex-1 flex flex-col gap-2 overflow-auto">
            ...
            <div class="flex items-center justify-between mb-1.5">
              <h3 class="text-xs font-semibold text-ink-muted uppercase tracking-wider flex items-center gap-1">
              ...
              <div class="flex gap-0.5">
                <button class="p-1 text-inv-border hover:text-ink-muted transition-colors">
            ...
            <div class="grid grid-cols-9 gap-0.5 w-fit">
              <div
                ...
                class="inv-slot"
                :class="[
                  item ? 'inv-slot-filled ' + getItemRarityBorder(item.itemId) : ''
                ]"
                @mouseenter="item && showTooltip(item, $event)"
                @mouseleave="hideTooltip"
              >
          ...
          <div class="inv-panel p-3">
            <h3 class="text-sm font-semibold text-ink-muted uppercase tracking-wider mb-2 flex items-center gap-1.5">
            ...
            <!-- Hotbar Label -->
            <h4 class="text-xs font-semibold text-ink-subtle uppercase tracking-wider mt-3 mb-1.5 flex items-center gap-1">
            ...
            <div class="grid grid-cols-9 gap-0.5 w-fit">
              <div
                ...
                :class="[
                  item ? 'inv-slot-filled ' + getItemRarityBorder(item.itemId) : '',
                  inventory?.activeHotbarSlot === index ? 'inv-slot-active' : ''
                ]"
                @mouseenter="item && showTooltip(item, $event)"
                @mouseleave="hideTooltip"
              >
                ...
                <!-- Slot number -->
                <span class="absolute top-0 left-0.5 text-[8px] font-bold text-inv-border/60">{{ index + 1 }}</span>
              </div>
            </div>
          </div>
          </div>
          ...
          <div v-else-if="activeTab === 'info'" class="flex-1 overflow-auto">
            <div class="inv-panel p-2">
              <h3 class="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">{{ t('avatarInventory.playerInfo') }}</h3>

              <div v-if="details" class="grid grid-cols-3 gap-2">
                <!-- World -->
                <div class="bg-inv-slot rounded p-2 border border-inv-border/20">
                  <div class="flex items-center gap-1 text-ink-subtle text-[10px] mb-0.5">
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-inv-slot rounded p-2 border border-inv-border/20">
                  ...
                  <p class="text-ink font-medium font-mono text-[10px]">
                    ...
                    <span v-else class="text-inv-border">-</span>
                  </p>
                </div>
              ...
              <div v-else class="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p class="text-ink-subtle text-sm">{{ t('avatarInventory.noInfoData') }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'chat'" class="flex-1 overflow-auto">
            <div class="inv-panel p-2 h-full flex flex-col">
              <h3 class="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">{{ t('avatarInventory.chatHistory') }}</h3>
              ...
              <div v-else-if="chatMessages.length === 0" class="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p class="text-ink-subtle text-sm">{{ t('avatarInventory.noChat') }}</p>
              </div>

              <!-- Chat messages -->
              <div v-else class="flex-1 space-y-1 overflow-y-auto">
                <div class="text-[10px] text-inv-border mb-1">
                  {{ chatTotal }} {{ t('avatarInventory.messages') }}
                </div>
                <div
                  v-for="msg in chatMessages"
                  :key="msg.id"
                  class="p-1.5 bg-inv-slot rounded border border-inv-border/15"
                >
                  ...
                  <p class="text-ink text-xs break-words">{{ msg.message }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'deaths'" class="flex-1 overflow-auto">
            <div class="inv-panel p-2 h-full flex flex-col">
              <h3 class="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">{{ t('avatarInventory.deathLocations') }}</h3>
              ...
              <div v-else-if="deathPositions.length === 0" class="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p class="text-ink-subtle text-sm">{{ t('avatarInventory.noDeaths') }}</p>
              </div>
              ...
              <div v-else class="flex-1 space-y-1.5 overflow-y-auto">
                ...
                <div
                  ...
                  :key="death.id"
                  class="p-2 bg-inv-slot rounded border border-inv-border/30 hover:border-red-500/30 transition-colors"
                >
                  <div class="flex items-center justify-between gap-2">
                    <div class="flex items-center gap-2 min-w-0">
                      ...
                      <div class="min-w-0">
                        ...
                        <div class="text-[10px] text-ink-subtle font-mono truncate">
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div
          v-if="hoveredItem"
          class="fixed z-[100] bg-inv-bg/95 border-2 rounded-lg shadow-2xl p-3 pointer-events-none min-w-[200px] max-w-xs backdrop-blur-sm"
        ...
        >
          ...
          <div class="text-[10px] text-inv-border font-mono mb-2 truncate">{{ hoveredItem.itemId }}</div>
          ...
          <div class="space-y-1.5 text-sm border-t border-inv-border/30 pt-2">
            ...
            <div v-if="hoveredItem.maxDurability > 0">
              ...
              <div class="h-1.5 bg-inv-slot rounded-full overflow-hidden border border-inv-border/20">
